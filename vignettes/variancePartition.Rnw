...
# Figure 1a
# Bar plot of variance fractions for the first 12 genes
plotPercentBars( varPart[1:12,] )
...
At the heart of \Rcode{variancePartition}, a regression model is fit for each gene separately and summary statistics are extracted and reported to the user for visualization and downstream analysis.  For a single model fit, \Rcode{calcVarPart} computes the fraction of variance explained by each variable.  \Rcode{calcVarPart} computes these statistics from either a fixed effects model fit with \Rcode{lm} or a linear mixed model fit with \Rcode{lmer}.  \Rcode{fitExtractVarPart} loops over each gene, fits the regress model and returns the variance fractions reported by \Rcode{calcVarPart}.
...
\begin{eqnarray}
  y_{i,s,c} = \mu + Z^{(sex)} \alpha_{i,s} + Z^{(Tcell| id)} \alpha_{i,c} + Z^{(monocyte| id)} \alpha_{i,c} + e_{i,s,c}
\end{eqnarray}    
