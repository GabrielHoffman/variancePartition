...
\\
Alternatively, the user can use the \Rcode{colinearityScore} function to evaluate whether this a an issue for a single model fit:  
...
See Section \ref{sec:variationWithAcross} for statistical details.
...
\begin{eqnarray}
  y_{i,s,c} = \mu + Z^{(sex)} \alpha_{i,s} + Z^{(T-cell| id)} \alpha_{i,c} + Z^{(monocyte| id)} \alpha_{i,c} + e_{i,s,c}
\end{eqnarray}    
...
\begin{center}
  \begin{tabular}{cc}
    ...
    \hline
    $\sigma^2_{sex}$ & variance across sex \\
    $\sigma^2_{(T-cell| id)}$ & variance across individuals within T-cells \\
...
plotStratifyBy( data, "Sex", "expression", ylim=c(-6, 9)) +
 annotate("text", x = 1, y = 8, label = paste("fold change:", format(coef(fit)[2], digits=3))) +
 annotate("text", x = 1.5, y = 7.2, label = paste("% variance of expression:", format(calcVarPart(fit)[1]*100, digits=3), "%"))


n = 500
data = data.frame(Sex = c(rep('F', n), rep('M', n)))
data$expression = rnorm(2*n, (as.integer(data$Sex)-1)*2, 2)
data$Sex = factor(data$Sex)
...
plotStratifyBy( data, "Sex", "expression", ylim=c(-6, 9)) +
 annotate("text", x = 1.0, y = 9, label = paste("fold change:", format(coef(fit)[2], digits=3))) +
 annotate("text", x = 1.5, y = 8.2, label = paste("% variance of expression:", format(calcVarPart(fit)[1]*100, digits=3), "%"))
@
...
\begin{itemize}
  ...
  \item \Rcode{In vcov.merMod(fit) : Computed variance-covariance matrix problem: not a positive definite matrix; returning NA matrix}
...
\subsection{Problems removing samples with NA/NaN/Inf values}
