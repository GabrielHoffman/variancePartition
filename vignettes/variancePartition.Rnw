---
title: "1) Tutorial on using variancePartition"
...
%\VignetteIndexEntry{variancePartiition}
%\VignettePackage{variancePartition}
...
options(digits=3, width=100, prompt=" ", continue=" ")
@
...
# If categorical variables are specified, a
# 	linear mixed model is used
# If all variables are modeled as fixed effects, 
#	a linear model is used
# each entry in results is a regression model fit on a single gene
# 2) extract variance fractions from each model fit for each gene, 
# returns fraction of variation attributable to each variable 
# Interpretation: the variance explained by each variables after 
# correcting for all other variables
...
\begin{figure}[hb]
\centering
\caption{ \Rpackage{variancePartition} example on simulated data}
...
head(varPart$Individual,3)

# sort genes based on variance explained by Individual
head(varPart[order(varPart$Individual, decreasing=TRUE),],3)
@
\newpage
...
# create data.frame with expression of gene i and Tissue type 
#	for each sample
...
GE <- data.frame( Expression = geneExpr[i,], Individual = 
	info$Individual)
...
\subfloat[Tissue]{\includegraphics[width=.45\textwidth]{figure/plotStratify-1}}
...
\subsection{Evaluating correlation between variables}

Since the correlation between variables can affect the results of variancePartition, it is important to evaluate the correlations in empirical data.  Evaluating the correlation between two vectors is straight-forward using the \Rcode{cor} function, and the impact of correlated variables in regression models is well understood.  However, \Rpackage{variancePartition} also considers discrete-valued variables with multiples levels (i.e. categories).  In order to evaluate the relationship between two discrete-valued variables, we can use canonical correlation analysis (CCA).  Note that CCA is a generalization of correlation, so that CCA run on two continuous variables gives the same results as CCA

Computing the pairwise correlation between all variables is simple with \Rcode{canCorPairs}.

<<canCorPairs, cache=TRUE, results='hide', fig.height=7, fig.width=7>>=
# specify formula
...
# this reports the correlation matrix between coefficients 
...
# In this example the weights are simulated from a uniform 
#	distribution and are not meaningful.
...
<<siteDominant, echo=FALSE, cache=TRUE, results='hide', fig.height=4, fig.width=4>>=
set.seed(1)
...
<<IndivDominant, echo=FALSE, cache=TRUE, results='hide', fig.height=4, fig.width=4>>=
set.seed(1)
...
\subfloat[PCA - colored by Tissue]{\includegraphics[width=.45\textwidth]{figure/siteDominant-1}}
\subfloat[PCA - colored by Individual]{\includegraphics[width=.45\textwidth]{figure/siteDominant-2}}\\
\subfloat[hclust - colored by Tissue]{\includegraphics[width=.45\textwidth]{figure/siteDominant-3}}
\subfloat[hclust - colored by Individual]{\includegraphics[width=.45\textwidth]{figure/siteDominant-4}}\\
\subfloat[variancePartition]{\includegraphics[width=.45\textwidth]{figure/siteDominant-5}}
\subfloat[variancePartition - within Tissue]{\includegraphics[width=.45\textwidth]{figure/siteDominant-6}}
...
\subfloat[PCA - colored by Tissue]{\includegraphics[width=.45\textwidth]{figure/IndivDominant-1}}
\subfloat[PCA - colored by Individual]{\includegraphics[width=.45\textwidth]{figure/IndivDominant-2}}\\
\subfloat[hclust - colored by Tissue]{\includegraphics[width=.45\textwidth]{figure/IndivDominant-3}}
\subfloat[hclust - colored by Individual]{\includegraphics[width=.45\textwidth]{figure/IndivDominant-4}}\\
\subfloat[variancePartition]{\includegraphics[width=.45\textwidth]{figure/IndivDominant-5}}
\subfloat[variancePartition - within Tissue]{\includegraphics[width=.45\textwidth]{figure/IndivDominant-6}}
...
<<DE, echo=FALSE, cache=TRUE, fig.height=4, fig.width=4>>=
set.seed(1)
...
\subfloat[Example A]{\includegraphics[width=.45\textwidth]{figure/DE-1}}
\subfloat[Example B]{\includegraphics[width=.45\textwidth]{figure/DE-2}}
\label{fig:DEVP}
\end{figure}
