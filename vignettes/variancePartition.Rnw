...
\begin{itemize}
 \item[1)] {\bf Gene expression data:}
 	...
 	For RNA-seq data, this can be the $\log_2$ counts per million (CPM) from \Rcode{cpm()} in the \Rpackage{edgeR}.  Or the output of \Rcode{voom()} in \Rpackage{limma} can be used to model the precision of each observation.  Other normalization methods such as $\log_2$ RPKM can also be used.  For microarray data, any standard normalization such as \Rcode{rma} can be used.    \\   
