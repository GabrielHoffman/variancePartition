...
Otherwise dream uses the same workflow as limma with `eBayes()` and `topTable()`, since any statistical differences are handled behind the scenes.
...
* `dream()` computes standard p-values for each gene.  Empirical Bayes procedure is then applied using `eBayes()` 
...
fit = dream( vobjDream[1:10,], form, metadata, L)
