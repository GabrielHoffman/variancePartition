...
isexpr = rowSums(cpm(countMatrix)>200) >= 10
...
# Here, get standard results
topTable( fitmm, coef='Disease1', number=3 )

# Get results using Empirical Bayes moderated t-statistics 
topTable( fitmmEB, coef='Disease1', number=3 )
```
Notes

* if a random effect is not specified, `dream()` automatically uses `lmFit()`.  
* if `lmFit()` is called, `eBayes()` must be used to be compatiable with limma
  - if a random effect is used, the empircal Bayes step is optional
...
fit = dream( vobjDream[1:10,], form, metadata, L, BPPARAM=param)
...
topTable( fiteb, coef="L1", number=3 )
```
...
# extract results from first contrast
topTable( fiteb, coef=c("DiseaseSubtype2", "DiseaseSubtype1"), number=3 )
```

## Small-sample method
For small datasets, the Kenward-Roger method can be more powerful.  But it is **substantially** more computationally intensive.
```{r kr, eval=FALSE}
fitmmKR = dream( vobjDream, form, metadata, ddf="Kenward-Roger", BPPARAM=param)
fitmmKREB = eBayes( fitmmKR )
```
...
vp = fitExtractVarPartModel( vobj, form, metadata, BPPARAM=param)
...
p2 = topTable(fitmmEB, number=Inf, sort.by="none")$P.Value
