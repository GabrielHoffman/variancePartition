...
L = makeContrastsDream( form, metadata, 
  contrasts = c("DiseaseSubtype2 - DiseaseSubtype1", "DiseaseSubtype1 - DiseaseSubtype0"))
...
topTable( fit, coef="DiseaseSubtype2 - DiseaseSubtype1", number=3 )
```

### Comparing multiple coefficients

So far contrasts have only involved the difference between two coefficients.  But contrasts can also compare any linear combination of coefficients.  Here, consider comparing `DiseaseSubtype0` to the mean of `DiseaseSubtype1` and `DiseaseSubtype2`

```{r maual.contrasts, fig.width=8, fig.height=4}
L2 = makeContrastsDream( form, metadata, contrasts = "DiseaseSubtype0 - (DiseaseSubtype1 + DiseaseSubtype2)/2")
...
topTable(fit, coef="DiseaseSubtype0 - (DiseaseSubtype1 + DiseaseSubtype2)/2", number=3)
```
