...
Report fraction of variance attributable to each variable
...
list() of where each entry is a model fit produced by
[`lmer()`](https://rdrr.io/pkg/lme4/man/lmer.html) or
[`lm()`](https://rdrr.io/r/stats/lm.html)
...
If there are no random effects, so formula is ~ a + b + c, a 'standard'
